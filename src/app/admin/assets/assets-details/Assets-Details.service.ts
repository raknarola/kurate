...
export class AssetsDetailsService {
  ...
  arrayForSaveMetaFields: Array<{ asset_id: number, meta_field_id: number, value: any }> = new Array<{ asset_id: number, meta_field_id: number, value: any }>();
  ...
  addMetaVlues(assetID: number) {
    ...
    // console.log(this.arrayForSaveMetaFields);
  ...
  getAssetsDetails(idOfAsset) {
    ...
    this.utilsService.postMethodAPI(false, this.utilsService.serverVariableService.getAssetDetailsAPI, formData, (response) => {
      if (!this.utilsService.isNullUndefinedOrBlank(response)) {
        ...
        // console.log(indexForRootFolder);
        ...
        // console.log(this.assetObj.user_tags);
  ...
  download(docId, docName) {
    ...
    this.http.post(UtilsService.URL + this.utilsService.serverVariableService.downloadIndividualAssetAPI, formData, { headers: headers })
      .subscribe(res => {
        if (res['response'] === 0) {
          this.utilsService.loaderStart--;
